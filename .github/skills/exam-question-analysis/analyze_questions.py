...
def main():
    ...
    print(f"- 线段树节点概要: {result['tree_overview_path']}")
    print(f"- 线段树可视化: {result['tree_visualization_path']}")
    print(f"- 三 Agent 分组概要: {result['agent_group_summary_path']}")
    print(f"- 三 Agent 分析文件: {len(result['agent_report_files'])} 个")
    print(f"- 三 Agent 工作进度数据: {result['agent_progress_json_path']}")
    print(f"- 三 Agent 工作进度可视化: {result['agent_progress_html_path']}")
