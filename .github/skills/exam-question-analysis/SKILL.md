...
- CLI 模式：适合直接执行、自动化处理、批量运行
- Web 模式：适合用户希望通过浏览器填写路径、查看状态、临时手动操作

如果用户没有明确说明：

- 默认优先使用 CLI
- 但如果用户明显在找“页面”“界面”“浏览器操作”“表单填写”的方式，要主动询问或切换到 Web 模式

如果启用了 agents 协作，这一步仍由协调者负责，不要把“CLI 还是 Web”的最终选择完全交给子 agent。
...
1. 先判断是否有理由走 Web 模式，否则默认 CLI
2. 再看是否已给图片目录
3. 如果没给，就提示需要图片目录
4. 如果仓库中已有测试图片，可说明先用测试目录演示
5. 再决定走本地 OCR 还是多模态 OCR
